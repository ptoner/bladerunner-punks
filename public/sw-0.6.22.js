...
//@ts-nocheck
const { DOMParser, XMLSerializer } = __webpack_require__(/*! @xmldom/xmldom */ "./node_modules/@xmldom/xmldom/lib/index.js");
...
self.addEventListener('fetch', event => {
    ...
    if (url.pathname.startsWith(`${baseURI}large`))
        process = false;
    if (url.pathname.startsWith(`${baseURI}sync`))
        process = false;
    if (url.pathname.startsWith(`${baseURI}t/`) && url.pathname.endsWith(`.json`))
        process = false;
...
const updateResponse = async (response) => {
    ...
    let content = he__WEBPACK_IMPORTED_MODULE_0___default().unescape(new XMLSerializer().serializeToString(pageElement));
    let component = `
        <template>
            ${content}
        </template>
...
/******/ })()
;
//# sourceMappingURL=sw-0.6.22.js.map